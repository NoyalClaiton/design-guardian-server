...
> **Note:** The plugin's allowed network connections are fixed in the plugin manifest. It can only communicate with `localhost:3001` to `3010`, `design-guardian.local:3001` to `3010`, and the hosted cloud URL above. You cannot deploy to your own hosting provider and have the plugin reach it.
...
**Step 3.** Install mkcert, install dependencies, and start the server:
...
The server generates a trusted HTTPS certificate automatically on first run. Once started, the terminal prints the exact URL to copy into the plugin settings (for example, `https://localhost:3001`).

### Notes

- The plugin only connects on ports **3001 to 3010**. If 3001 is taken the server tries the next port automatically.
...
**Step 2.** Follow the same setup steps as Option 1 (download, `.env`, install mkcert, `npm start`). The server generates a certificate that covers `design-guardian.local` automatically.
...
If the Claude CLI is not installed, connector-based fetching is skipped cleanly and the scan continues with direct HTTP only. If a page cannot be fetched (connector not enabled, access not granted), that URL is skipped and retried after 10 minutes.
...
|---|---|---|
| `PORT` | `3001` | Starting port. The server tries 3001 to 3010 in order and uses the first available one. |
